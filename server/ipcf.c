...
void c_packet(char *pkt)
{
	...
	mdb("server erroneously got client packet");
}
...
void c_didpoll(void)
{
	...
	mdb("server erroneously got client didpoll call");
}


void c_lostcon(void)
{
	mdb("server erroneously got client lost connection packet");
}
	...
	if (n == 1) {
		...
		for(i=0;i<MAX_GROUPS;i++)
		{
		    ...
		    if ( g_tab[i].control != PUBLIC && g_tab[i].mod >= 0 )
			{
				...
				if ( mod_idle > g_tab[i].idlemod )
				{
					 ...
					 * in this group who are less idle that the mod.
					...
					{
						char *modmsg = IDLE_MOD_MSG;

						/* let them know */
						sprintf (mbuf, modmsg, "you");
		...
		for(i=0;i<MAX_REAL_USERS;i++) {
			...
			if((TheTime - u_tab[i].t_recv) > MAX_IDLE) {
			    ...
			    sprintf(mbuf,
			    "[TIMEOUT] %d (%ld - %ld > %d)", i,
				TheTime, u_tab[i].t_recv, MAX_IDLE);
			    mdb(mbuf);
			    sendstatus(i, "Drop", 
...
{
	mdb("[ERROR] server erroneously got client userchar call");
}
