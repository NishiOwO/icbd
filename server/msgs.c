...
long get_perms (int n, char *login)
{
    ...
    {
	sprintf(mbuf, "[ERROR] Permissions File Open: %s", strerror(errno));
	mdb(mbuf);
	return (PERM_NULL);
    }
...
void openmsg(int n, char *pkt)
{
	...
	int gi;
		...
		if (split(pkt) != 1) {
			mdb("[ERROR] got bad open message packet");
		} else {
#ifdef VERBOSE
		   sprintf(mbuf,"[OPEN] %d", n);
		   mdb(mbuf);
#endif
	    ...
	    sprintf (mbuf,
		"[ERROR] %d: cannot send open messages until logged in (%s)",
		n, fields[0]);
	    mdb(mbuf);
	}
}
...
int loginmsg(int n, char *pkt)
{
	...
	if ( num_fields >= 3 && fields[2][0] != '\0' )
	    sprintf (fields[1], "%s\000", fields[2]);
	else
	    fields[1][0] = '\000';
	...
	else
	{
		...
		senderror(n, mbuf);
		mdb (mbuf);
		sprintf (mbuf, "Email %s for details.", SERVER_ADMIN);
		senderror(n, mbuf);
		return (-1);
	    }
	    ...
	    {
		mbuf[0] = '\0';
		if ( perms & PERM_SLOWMSGS ) { strcat (mbuf, "slowmsgs"); }
	...
	if (access_file >= 0)
	{
		...
		if (isspace(c))
		{
			...
			{
			    sprintf(mbuf, "[ERROR] Access File Close: %s",
				strerror(errno));
			    mdb(mbuf);
			}
			return -1;
	...
	{
	    sprintf(mbuf, "[ERROR] Access File Open: %s", strerror(errno));
	    mdb(mbuf);
	}
		
	/* get rid of nasty characters in the nickname */
	/* mdb("filtering nickname"); */
	filternickname(fields[1]);

	/* fill in what we can */
	/* mdb("filling in user entry"); */
...
void cmdmsg(int n, char *pkt)
{
	...
	{
		sprintf (mbuf,
		    "[ERROR] %d: cannot issue command until logged in (%s)",
		    n, fields[0]);
		mdb(mbuf);
	}
}
...
void pong(int n, char *pkt)
{
	char tbuf[255];
		...
		if ( pong_req[n] != -1 )
		{
		 ...
		 mdb("[ERROR] cannot send pong message until signed on");
	}
}
