...
int s_help(int n, int argc)
{
    ...
    if (help_fd >= 0) {
	    ...
	    if (close(help_fd) != 0) {
		    sprintf(mbuf, "[ERROR] Help File Close: %s",
			    strerror(errno));
		    mdb(mbuf);
		    }
    } else {
	    sprintf(mbuf, "[ERROR] Help File Open: %s",
		    strerror(errno));
	    mdb(mbuf);
...
/*			mdb("person is unknown"); */
		}

		for (i = 0; i < MAX_GROUPS; i++)
		    talk_report (n, i);

/*		mdb("finished checking on person"); */
        } else {
                mdb("name: wrong number of parz");
        }
       return 0;
}
       ...
       if (argc == 2) {
		if(strcasecmp(fields[1],"off") == 0) {
			...
			senderror(n,"Echoback: needs on/off/verbose");
                ...
                mdb("echoback: wrong number of parz");
        }
       	return 0;
}
       ...
       if (argc == 2) {
		if(strcasecmp(fields[1],"off") == 0) {
			...
			senderror(n,"No-Beep: needs off/on/verbose");
                ...
                mdb("No-Beep: wrong number of parz");
        }
       return 0;
}
...
int s_ping (int n, int argc)
{
    ...
    if ( argc != 2 )
    {
	mdb("s_ping: wrong number of parz");
	return (-1);
    }
...
int s_noaway(int n, int argc)
{
	if (argc == 0 || argc > 2)
	{
                mdb("NoAway: wrong number of parz");
		return -1;
	}
...
{
	struct tm *t;
	char buf[MAX_INPUTSTR];

	if (argc == 0 || argc > 2)
	{
                mdb("Away: wrong number of parz");
		return -1;
	}
	...
	if ( strlen (fields[1]) == 0 )
	{
		...
		sprintf (mbuf, "Away message is set to \"%s\"",
		    u_tab[n].awaymsg);
		snprintf(buf, MAX_INPUTSTR, mbuf, u_tab[n].nickname);
		sendstatus(n, "Away", buf);
	    }
	    else
