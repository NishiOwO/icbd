...
int add_item(int n, char* item, char *buf)
{
	if ((strlen(item) + 1) >= MAX_LINE) {
		mdb("[ERROR] Attempted to add_item something too big");
		return -1;
		}
...
/*	mdb("substitute n's group"); */
			}
/*	mdb("one particular group"); */
			doOne(n, flags, tgrp, 1);
		}
	} else {
/*		mdb("who: wrong number of parz"); */
	}
